...
class CliTests(unittest.TestCase):
    ...
    def test_upgrade_plan_is_additive_json_output(self):
        machine = MachineInfo("Test", "AMD64", "Test CPU", 4, 3.0, 16, 500, 200)
        output = io.StringIO()
        with patch("cli.collect_machine_info", return_value=machine), redirect_stdout(output):
            self.assertEqual(cli.main(["--check", "Windows 11", "--upgrade-plan", "--json"]), 0)
        payload = json.loads(output.getvalue())
        self.assertIn("upgrade_plan", payload["results"][0])
