...
class ReadinessApp(tk.Tk):
    ...
    def _render_analysis_details(self, sections: tuple[tuple[str, tuple[str, ...]], ...]) -> None:
        ...
        for index, (title, lines) in enumerate(sections):
            if index:
                tk.Frame(self.details_sections, bg=UI["subtle_border"], height=1).pack(fill="x", pady=(14, 8))
            tk.Label(self.details_sections, text=title, bg=UI["background"], fg=UI["text"], font=(self.font, 9, "bold"), anchor="w").pack(fill="x", pady=(0, 4))
            for line in lines:from __future__ import annotations
...
class ReadinessApp(tk.Tk):
    ...
    def show_recommendations(self) -> None:
        ...
        self._size_dialog(window, 700, 680, 560, 480)
        window.configure(bg=UI["background"])
        scroll = ScrollableWorkspace(window, background=UI["background"])
        scroll.pack(fill="both", expand=True, padx=PAGE_PADDING[0], pady=PAGE_PADDING[1])
        card = FluentCard(scroll.content, tokens=self.ui_tokens, padding=DIALOG_PADDING)
    ...
    def _render_analysis_details(self, sections: tuple[tuple[str, tuple[str, ...]], ...]) -> None:
        ...
        for index, (title, lines) in enumerate(sections):
            ...
            tk.Label(self.details_sections, text=title, bg=UI["background"], fg=UI["text"], font=(self.font, 9, "bold"), anchor="w").pack(fill="x", pady=(0, 4))
            for line in lines:
                tk.Label(self.details_sections, text="• " + line, bg=UI["background"], fg=UI["muted"], font=(self.font, 9), anchor="w", justify="left", wraplength=640).pack(fill="x", padx=(10, 0), pady=(3, 0))
        self.details_sections.pack(fill="x", pady=(8, 0))
    ...
    def show_machine_compare(self) -> None:
        ...
        for index, title in enumerate(machine_names):
            ...
            actions = tk.Frame(source_body, bg=UI["surface"])
        ...
        table.heading("#0", text=t("comparison.attribute", self.language), anchor="w"); table.heading("a", text=machine_names[0], anchor="w"); table.heading("b", text=machine_names[1], anchor="w"); table.heading("difference", text=t("comparison.difference", self.language), anchor="w")
        table.column("#0", width=180, minwidth=150, anchor="w", stretch=False); table.column("a", width=240, minwidth=180, anchor="w", stretch=True); table.column("b", width=240, minwidth=180, anchor="w", stretch=True); table.column("difference", width=180, minwidth=150, anchor="w", stretch=True)
        table.pack(fill="both", expand=True)
